...
static struct termios old, new_g;

/* Initialize new terminal i/o settings */
void initTermios(int echo) 
{
  tcgetattr(0, &old); /* grab old terminal i/o settings */
  new_g = old; /* make new settings same as old settings */
  new_g.c_lflag &= ~ICANON; /* disable buffered i/o */
  new_g.c_lflag &= echo ? ECHO : ~ECHO; /* set echo mode */
  tcsetattr(0, TCSANOW, &new_g); /* use these new terminal i/o settings now */
}
...
/* TNTAntoine specific workaround */
char _getch(void) 
{
  return getch_(0);
}
